...
type KnowledgeSearchResult = {
  ...
  careGuideCoverageGenuses: string[];
};
...
async function getAvailableCareGuideGenuses(): Promise<string[]> {
  const rows = await sql`
    select distinct pet_genus
    from public.pet_care
    where pet_genus is not null
      and btrim(pet_genus) <> ''
    order by pet_genus asc
  `;

  return normalizeTerms(
    rows
      .map((row: any) => row.pet_genus)
      .filter((genus: unknown): genus is string => typeof genus === "string"),
    100,
  );
}

function hasCareGuide(
  genus: unknown,
  availableCareGuideGenuses: string[],
): genus is string {
  if (typeof genus !== "string") return false;

  const normalizedGenus = genus.trim().toLowerCase();
  return availableCareGuideGenuses.some(
    (availableGenus) => availableGenus.trim().toLowerCase() === normalizedGenus,
  );
}
...
async function getCareGuidesByGenuses(
  genuses: string[],
  availableCareGuideGenuses: string[],
): Promise<any[]> {
  const coveredGenuses = normalizeTerms(genuses, 10).filter((genus) =>
    hasCareGuide(genus, availableCareGuideGenuses),
  );
...
async function searchKnowledge(
  ...
  const careGuideGenusesFound = possibleCareGuideGenuses.filter((genus) =>
    hasCareGuide(genus, availableCareGuideGenuses),
  );
  const careGuideGenusesMissing = possibleCareGuideGenuses.filter(
    (genus) => !hasCareGuide(genus, availableCareGuideGenuses),
  );
  ...
  const careGuideRows = shouldRetrieveCareGuides
    ? await getCareGuidesByGenuses(
        careGuideGenusesFound,
        availableCareGuideGenuses,
      )
    : [];
  ...
  return {
    ...
    careGuideCoverageGenuses: availableCareGuideGenuses,
  };
}
...
function buildKnowledgeContext(
  ...
  const careGuideCoverage =
    knowledge.careGuideCoverageGenuses.length > 0
      ? `Care guide coverage: Shell & Fin currently has genus-level care guides for ${knowledge.careGuideCoverageGenuses.join(", ")}.`
      : "Care guide coverage: Shell & Fin currently has no genus-level care guides listed in public.pet_care.";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const availableCareGuideGenuses = await getAvailableCareGuideGenuses();
    const knowledge = await searchKnowledge(analysis, availableCareGuideGenuses);
